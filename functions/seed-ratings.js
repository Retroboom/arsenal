/**
 * Seed Elo ratings — Cloudflare Pages Function
 * URL: https://arsenal.retroboomgames.com/seed-ratings?token=<KOFI_VERIFICATION_TOKEN>
...
import { getFirebaseToken, fbGet, fbSet } from './_firebase.js';
...
export async function onRequestGet(context) {
  ...
  if (check) {
    const existing = await fbGet(fbToken, 'ratings');
    const existingKeys = existing ? Object.keys(existing).length : 0;
    const seedKeys = Object.keys(SEEDED).length * QUALITIES.length;
    const sample = existing ? Object.entries(existing).slice(0, 5) : [];
    return new Response(JSON.stringify({ existingKeys, seedKeys, sample }, null, 2), {
      status: 200,
  ...
  await Promise.all(
    Object.entries(writes).map(([key, val]) => fbSet(fbToken, `ratings/${key}`, val)),
  );

  return new Response(
    JSON.stringify({ seeded: Object.keys(writes).length, teams: Object.keys(SEEDED).length }),
